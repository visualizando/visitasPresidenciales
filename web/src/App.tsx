...
import {useAudienciasDetails} from "./hooks/useAudienciasDetails";
...
import {analyticsForChartPeriod, eventsForChartPeriod} from "./utils/analyticsPeriod";
...
export default function App() {
  ...
  const audiencias = useAudienciasDetails(selected);
  ...
  const periodEvents = useMemo(() => eventsForChartPeriod(events.data, chartPeriod), [events.data, chartPeriod]);
  const selectionAnalytics = useMemo(() => aggregateEvents(periodEvents), [periodEvents]);
  const publishedAnalytics = useMemo(() => analyticsForChartPeriod(analytics.data, chartPeriod), [analytics.data, chartPeriod]);
  const activeAnalytics = selected.length ? selectionAnalytics : publishedAnalytics;
  ...
  return <>
    ...
    <main id="main">
      <section className="hero" aria-labelledby="page-title">
        <div className="hero-copy"><h1 id="page-title">Explorador de accesos a Olivos y Casa Rosada</h1><div className="hero-intro"><p className="hero-lead">Desde 2016, Poder Ciudadano pone a disposición los registros de ingresos a la Casa Rosada y la Quinta Presidencial de Olivos, obtenidos a partir de pedidos de acceso a la información pública. Este explorador facilita la consulta y búsqueda de las planillas de acceso, y también suma los datos del registro de audiencias de gestión de intereses.</p><p className="hero-source">La información es brindada por Casa Militar, organismo público a cargo de la seguridad presidencial, por lo que Poder Ciudadano no es responsable por su contenido.</p><details className="hero-more"><summary>Ver más sobre cómo se arma esta base<ChevronDown aria-hidden="true" /></summary><div className="hero-more-content"><p>Para poder mostrar estos datos, Poder Ciudadano hace pedidos de acceso a la información pública cada seis meses y procesa la respuesta, que suele llegar en un PDF con tablas. Esos archivos se leen y se vuelcan a este explorador. Lo mismo se hace con el registro de audiencias de gestión de intereses.</p><p>Para armar el listado de personas se unifican datos y nombres en base a coincidencias (mismo documento, mismo nombre, mismas variantes). Es un proceso automático que puede tener errores: si un dato figura acá es porque está en la fuente original, pero si no figura puede ser que esté mal escrito o que las autoridades no lo hayan cargado.</p><p>Idealmente nada de esto debería ser necesario: esta información debería ser abierta, y ser provista mediante transparencia activa y útil por el propio gobierno.</p></div></details></div></div>
      ...
      <section className="search-section" id="buscar" aria-labelledby="search-title">
        ...
        {selected.length > 0 && <PersonProfile people={selected} events={events.data} audiencias={audiencias.details} loading={events.loading} error={events.error} coincidences={coincidences.data} onRemove={(entityId) => setSelected((current) => current.filter((person) => person.entity_id !== entityId))} onClear={() => setSelected([])} />}
      </section>

      <section className="dashboard-section" id="panorama" aria-labelledby="dashboard-title">
        <div className="section-heading dashboard-heading"><div><h2 id="dashboard-title">Actividad</h2></div><div className="dashboard-controls"><ChartPeriodFilter value={chartPeriod} onChange={setChartPeriod} /><ChartLocationFilter value={chartLocations} onChange={setChartLocations} /></div></div>
        ...
        {activeAnalytics && !dashboardLoading && !dashboardError && <div className="dashboard-grid"><div className="dashboard-wide"><CalendarChart data={activeAnalytics.daily} location={chartLocation} series={personSeries} mileiCasaRosadaDays={chartLocations.includes("casa-rosada") ? publishedAnalytics?.milei_casa_rosada_days : []} /></div><HeatmapChart data={activeAnalytics.heatmap} location={chartLocation} series={personSeries} /><PurposeChart data={purposeData} series={personSeries} /></div>}
      </section>

      <section className="downloads-section" id="descargas" aria-label="Descargar datos">
        <div className="more-disclosures"><details><summary><span><strong>Descargar datos</strong><small>Archivos CSV por año</small></span><ChevronDown aria-hidden="true" /></summary><div className="more-content downloads-content">{exportsData.data?.length ? <div className="download-table-wrap"><table className="download-table"><caption className="sr-only">Archivos de datos disponibles por año</caption><thead><tr><th scope="col">Año</th><th scope="col">Registros</th><th scope="col">Archivo</th></tr></thead><tbody>{[...exportsData.data].sort((left, right) => right.year - left.year).map((file) => <tr key={file.year}><td><strong>{file.year}</strong></td><td>{file.records.toLocaleString("es-AR")}</td><td><a href={new URL(`data/exports/${file.path}`, document.baseURI).href} download>Descargar CSV</a></td></tr>)}</tbody></table></div> : <p className="empty-state">Los CSV aparecerán después de la primera actualización de datos.</p>}</div></details></div>
      </section>
